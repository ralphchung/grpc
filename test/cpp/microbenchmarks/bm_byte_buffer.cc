...
static void BM_ByteBufferReader_Next(benchmark::State& state) {
  ...
  for (int i = 0; i < num_slices; ++i) {
    ...
    slices.emplace_back(
        CoreCodegen::grpc_slice_from_copied_buffer(buf.get(), kSliceSize));
  }
  grpc_byte_buffer* bb =
      CoreCodegen::grpc_raw_byte_buffer_create(slices.data(), num_slices);
  grpc_byte_buffer_reader reader;
  GPR_ASSERT(CoreCodegen::grpc_byte_buffer_reader_init(&reader, bb));
  for (auto _ : state) {
    grpc_slice* slice;
    if (GPR_UNLIKELY(
            !CoreCodegen::grpc_byte_buffer_reader_peek(&reader, &slice))) {
      CoreCodegen::grpc_byte_buffer_reader_destroy(&reader);
      GPR_ASSERT(CoreCodegen::grpc_byte_buffer_reader_init(&reader, bb));
      continue;
    }
  }
  ...
  CoreCodegen::grpc_byte_buffer_reader_destroy(&reader);
  CoreCodegen::grpc_byte_buffer_destroy(bb);
  for (auto& slice : slices) {
    CoreCodegen::grpc_slice_unref(slice);
  }
}
